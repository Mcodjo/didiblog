<x-layouts.app title="Accueil">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <h2 class="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-6">{{ $quiSuisJe['title'] }}</h2>
                    <p class="text-lg text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
                        {!! $quiSuisJe['description1'] !!}
                    </p>
                    <p class="text-lg text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
                        {!! $quiSuisJe['description2'] !!}
                    </p>
                    <div class="grid grid-cols-2 gap-6 mb-8">
                        <div class="text-center">
                            <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">{{ $quiSuisJe['stat1_value'] }}</div>
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ $quiSuisJe['stat1_label'] }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $quiSuisJe['stat2_value'] }}</div>
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ $quiSuisJe['stat2_label'] }}</div>
                        </div>
                    </div>
                ...
                <div class="relative">
                    <div class="bg-gradient-to-br from-orange-100 to-blue-100 dark:from-orange-900 dark:to-blue-900 rounded-2xl p-8">
                        <img src="{{ $quiSuisJe['image'] }}" alt="Coach Didi" class="w-full h-auto max-h-[400px] object-contain rounded-xl shadow-xl border-4 border-orange-500">
                    </div>
                </div>
            </div>
        </div>
    ...
    @if($articles->count())
    <section class="py-16 lg:py-20 bg-gray-100 dark:bg-gray-800">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                    <i class="fas fa-book-open text-orange-600 mr-3"></i>Plus d'Articles
                </h2>
                <p class="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
                    Continue ta lecture avec ces autres articles pour approfondir tes connaissances.
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                @foreach($articles as $article)
                <article class="bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
                    <img src="{{ $article->image_url ?? 'https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=500' }}" alt="{{ $article->titre }}" class="w-full h-48 object-cover">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <span class="px-3 py-1 rounded-full text-sm font-medium text-white" style="background-color: {{ $article->categorie->couleur ?? '#f97316' }}">{{ $article->categorie->nom ?? 'Général' }}</span>
            ...
            <div class="text-center">
                <a href="{{ route('blog.index') }}" class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-semibold rounded-lg transition-all duration-300">
                    <i class="fas fa-th-list mr-2"></i>Explorer tous les articles
                </a>
            </div>
        </div>
    </section>
